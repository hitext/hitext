...
/**
 * Creates range hooks for viewport/windowing scenarios with smart truncation indicators.
 *
 * This factory generates hooks that display truncation markers when content is hidden:
 * - `replace`: Used for completely skipped/hidden ranges (e.g., lines outside viewport)
 * - `close`: Used when content is trimmed from the start (range doesn't start at line start)
 * - `open`: Used when content is trimmed from the end (range doesn't end at line content end)
 *
 * The hooks intelligently handle multi-line ranges:
 ...
 * @param options.ellipsis - Content for trimmed ranges. Can be:
 ...
 *   Default: '...'
 * @returns Range hooks object with replace, open, and close hooks
 *
 * @example
 ...
 * html()
 *   .addLayer(hiddenRanges, rangeHooksHide({
 ...
 *   }))
 */
export function rangeHooksHide<T = unknown, R = T>(options: {
    ...
    return {
        ...
        replace(context) {
            ...
            // Determine if range should be replaced entirely (vs. partial replacement)
            const shouldReplace =
                // Multi-line ranges (2+ lines) are always replaced
                lineDiff >= 2 ||
            ...
            if (!shouldReplace) {
                // For partial line ranges, check if it spans multiple lines
                if (!lines.isSameLine(start, end)) {
                    // Multi-line partial range: preserve newline so close/open appear on separate lines
                    // Return the newline character(s) at the end of the start line
                    return lines.getNewlineText(start);
                } else {
                    // Single-line partial range: return single marker to avoid double markers
        ...
        close({ start, end, lines }) {
            // Add marker if: multi-line range and content trimmed from right
        ...
        open({ start, end, lines }) {
            // Add marker if: multi-line range and content trimmed from left
