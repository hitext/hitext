...
import { applyExpandTo, generateSpans } from '../../src/index.js';
...
describe('applyExpandTo', () => {
    describe('Expand positions', () => {
        it('should expand to line (including newlines)', () => {
            const expanded = renderSpans('line1\nline2\nline3', applyExpandTo('line')([[7, 9]]));
    ...
    describe('Data and origin', () => {
        it('should preserve data when expanding', () => {
            ...
            deepStrictEqual(startEndData(spans), [[6, 12, { type: 'match' }]]);
        });
    ...
    describe('Edge cases', () => {
        ...
        it('should handle multiline span', () => {
            const expanded = renderSpans('line1\nline2\nline3\nline4', applyExpandTo('line')([[7, 15]]));
            deepStrictEqual(expanded, ['line2\nline3\n']);
        });

        it('should handle span at document boundaries', () => {
            const start = renderSpans('line1\nline2\nline3', applyExpandTo('line')([[0, 2]]));
            const end = renderSpans('line1\nline2\nline3', applyExpandTo('line')([[15, 17]]));
            deepStrictEqual(start, ['line1\n']);
            deepStrictEqual(end, ['line3']);
        });

        it('should handle empty document', () => {
            const expanded = renderSpans('', applyExpandTo('line')([[0, 0]]));
            deepStrictEqual(expanded, ['']);
        });

        it('should handle multiple spans', () => {
            const spans = generateSpans('one\ntwo\nthree', applyExpandTo('document-start')([[0, 3], [8, 13]]));
            deepStrictEqual(startEnd(spans), [[0, 3], [0, 13]]);
        });
    });
});
